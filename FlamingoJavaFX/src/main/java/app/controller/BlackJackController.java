...
public class BlackJackController implements Initializable {
	...
	public void btnHit_Click(ActionEvent event) {
		...
		// a translate):
		TranslateTransition transT = CreateTranslateTransition(pntDeck, new Point2D(pntCardDealt.getX(), pntCardDealt.getY()) , img);
		...
		patTMoveRot.getChildren().addAll(transT,rotT);
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new CubicCurveTo(240, 240, 260, 240, toPoint.getX(), toPoint.getY()));
